...
let inStockProducts = filterProducts((product) => product.inStock);
let outOfStockProducts = filterProducts((product) => !product.inStock);
...
*/
let upperCasedProducts= products.map((products)=> products.name.toUpperCase())
...
function applyDiscount(discountPercent) {
  return (product) => {
    product.salesPrice = product.price -(product.price * discountPercent);
  };
};

products.forEach(applyDiscount(0.20));
...
*/
let totalValueInStock = products
  .filter(product => product.inStock)
  .reduce((total, product) => total + product.price, 0);

console.log("Total Inventory Value:", totalValueInStock);
